...
A etapa 2.1.A do plano mestre (`docs/plans/docs-implementation-master-plan.md`) exige a escolha de um layout oficial para o HUD antes de qualquer refatoração estrutural. As propostas anteriores — "Minimal Tactical HUD" e "Sleek Tactical HUD" — já atacavam problemas reconhecidos nas revisões de UI (cards ocupando cantos inteiros, duplicação de dados e excesso de rótulos). O objetivo desta decisão é consolidar uma direção única, alinhada com o código já existente, e preparar a implementação incremental dos itens 2.1.B e 2.1.C.
