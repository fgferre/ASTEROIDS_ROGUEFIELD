...
2. **Verificar Implementação**
   ...
   - Consultar históricos em `docs/progress/` e `historico_do_projeto.txt` para cruzar informações.
...
| Documento | Propósito | Como verificar | Entregável |
|-----------|-----------|----------------|------------|
| `completed-prompts.md` | Histórico de prompts já utilizados. | Checar se lista está atualizada com entregas recentes; cruzar com `historico_do_projeto.txt`. | Atualizar ou arquivar se duplicado. |
