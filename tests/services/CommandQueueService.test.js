...
describe('CommandQueueService', () => {
  ...
  it.concurrent(
    'enqueues commands with sequential frame tags and consumes them in order',
    () => {
  ...
  it.concurrent(
    'respects explicit frame overrides and filtering during consumption',
    () => {
      const queue = new CommandQueueService({ initialFrame: 0 });
      ...
      expect(remaining.map((entry) => entry.type)).toStrictEqual([
        'move',
        'firePrimary',
      ]);
    }
  ...
  it.concurrent(
    'invokes instrumentation hooks on enqueue, consume and clear',
    () => {
  ...
  it.concurrent(
    'creates deep snapshots of payloads and metadata to avoid mutation bleed',
    () => {
      ...
      expect(consumed[0].metadata).toStrictEqual({
        origin: { id: 'player-1' },
      });
    }
  );
