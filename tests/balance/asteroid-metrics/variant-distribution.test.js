...
import { SIZE_TEST_SAMPLES, WAVE_TEST_SAMPLES } from '../../__fixtures__/enemies.js';
  ...
  describe('Variant Distribution by Size', () => {
    ...
    SIZE_TEST_SAMPLES.forEach((size) => {
      test(`${size} variant mix matches availability-aware distribution`, () => {
        const results = sampleVariants(harness.enemySystem, size, 1, VARIANT_SAMPLE_COUNT);
        ...
        const expected = computeExpectedVariantBreakdown(harness.enemySystem, size, 1);
        ...
        for (const [variant, probability] of Object.entries(expected.probabilities)) {
    ...
    test('parasite remains unavailable before wave 4', () => {
      const results = sampleVariants(harness.enemySystem, 'medium', 3, VARIANT_SAMPLE_COUNT);
      expect(results.parasite ?? 0).toBe(0);
    });

    test('parasite participates in the distribution from wave 4 onward', () => {
      const results = sampleVariants(harness.enemySystem, 'medium', 4, VARIANT_SAMPLE_COUNT);
      const expected = computeExpectedVariantBreakdown(harness.enemySystem, 'medium', 4);
    ...
    test('gold variant never spawns for large asteroids', () => {
      const results = sampleVariants(harness.enemySystem, 'large', 6, VARIANT_SAMPLE_COUNT);
      expect(results.gold ?? 0).toBe(0);
    });

    test('denseCore variant absent for small asteroids', () => {
      const results = sampleVariants(harness.enemySystem, 'small', 6, VARIANT_SAMPLE_COUNT);
  ...
  describe('Variant Wave Scaling', () => {
    WAVE_TEST_SAMPLES.forEach((waveNumber) => {
      test(`medium asteroid special rate scales at wave ${waveNumber}`, () => {
        ...
        const total = Object.values(results).reduce((accumulator, value) => accumulator + value, 0);
        ...
        const tolerance = Math.max(0.003, Math.min(0.02, expected.specialChance * 0.2));
        expect(Math.abs(actualSpecial - expected.specialChance)).toBeLessThanOrEqual(tolerance);
      });
    });
  });
});
