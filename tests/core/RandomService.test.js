...
describe('RandomService determinism', () => {
  ...
  it.concurrent('produces identical float sequences for the same seed', () => {
    ...
    const floatsA = sampleSequence(randomA, 8, (rng) => rng.float()).map(
      (value) => Number(value.toFixed(12))
    );
    const floatsB = sampleSequence(randomB, 8, (rng) => rng.float()).map(
      (value) => Number(value.toFixed(12))
    );
  ...
  it.concurrent('produces identical range sequences for the same seed', () => {
    ...
    const rangesA = sampleSequence(randomA, 6, (rng) => rng.range(-25, 42)).map(
      (value) => Number(value.toFixed(10))
    );
    const rangesB = sampleSequence(randomB, 6, (rng) => rng.range(-25, 42)).map(
      (value) => Number(value.toFixed(10))
    );
  ...
  it.concurrent(
    'selects identical elements with pick for the same seed',
    () => {
      const seed = 424242;
      const randomA = new RandomService(seed);
      const randomB = new RandomService(seed);
  ...
  it.concurrent(
    'selects identical elements with weightedPick for the same seed',
    () => {
      const seed = 777;
      const randomA = new RandomService(seed);
      const randomB = new RandomService(seed);
      ...
      const picksA = sampleSequence(randomA, 10, (rng) =>
        rng.weightedPick(weights)
      );
      const picksB = sampleSequence(randomB, 10, (rng) =>
        rng.weightedPick(weights)
      );
  ...
  it.concurrent('generates identical UUIDs for the same seed', () => {
    ...
    const uuidsA = sampleSequence(randomA, 5, (rng, index) =>
      rng.uuid(`scope-${index}`)
    );
    const uuidsB = sampleSequence(randomB, 5, (rng, index) =>
      rng.uuid(`scope-${index}`)
    );
  ...
  it.concurrent('fork produces deterministic child generators', () => {
    ...
    const captureForkData = (rng) => {
      ...
      const spawnData = {
        floats: sampleSequence(spawnFork, 4, (fork) => fork.float()).map(
          (value) => Number(value.toFixed(8))
        ),
        uuids: sampleSequence(spawnFork, 2, (fork, index) =>
          fork.uuid(`spawn-${index}`)
        ),
      };

      const nestedFork = lootFork.fork('drops.special');
      const lootData = {
        ranges: sampleSequence(lootFork, 3, (fork) => fork.range(0, 100)).map(
          (value) => Number(value.toFixed(8))
        ),
        nestedFloats: sampleSequence(nestedFork, 3, (fork) => fork.float()).map(
          (value) => Number(value.toFixed(8))
        ),
      };

      const rootContinuation = sampleSequence(rng, 3, (root) =>
        root.float()
  ...
  it.concurrent('repeats sequences after resetting to the same seed', () => {
    ...
    const executeRun = (rng) => ({
      floats: sampleSequence(rng, 8, (service) =>
        Number(service.float().toFixed(12))
      ),
      ranges: sampleSequence(rng, 8, (service) =>
        Number(service.range(0, 100).toFixed(10))
      ),
      picks: sampleSequence(rng, 8, (service) => service.pick(values)),
      uuids: sampleSequence(rng, 8, (service, index) =>
        service.uuid(`reset-${index}`)
      ),
    });
  ...
  it.concurrent('serialize and restore preserves deterministic state', () => {
    ...
    const consumedFloats = sampleSequence(random, 5, (rng) => rng.float()).map(
      (value) => Number(value.toFixed(12))
    );
    ...
    const restoredFloats = sampleSequence(restored, 5, (rng) =>
      Number(rng.float().toFixed(12))
    );

    const reference = new RandomService(seed);
    sampleSequence(reference, 5, (rng) => rng.float());
    const referenceFloats = sampleSequence(reference, 5, (rng) =>
      Number(rng.float().toFixed(12))
    );
