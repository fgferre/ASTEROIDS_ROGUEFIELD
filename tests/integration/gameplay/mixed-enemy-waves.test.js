...
function createWaveManager(overrides = {}) {
  ...
  const random =
    overrides.random ?? createDeterministicRandom({ floatValue: 0.5 });
  ...
  it('unlocks support enemies at configured progression thresholds', () => {
    withWaveOverrides(
      ...
      () => {
        ...
        expect(droneWave.enemies.some((group) => group.type === 'drone')).toBe(
          true
        );
        expect(droneWave.enemies.some((group) => group.type === 'mine')).toBe(
          false
        );
        ...
        expect(mineWave.enemies.some((group) => group.type === 'drone')).toBe(
          true
        );
        expect(mineWave.enemies.some((group) => group.type === 'mine')).toBe(
          true
        );
        expect(mineWave.enemies.some((group) => group.type === 'hunter')).toBe(
          false
        );
        ...
        expect(
          hunterWave.enemies.some((group) => group.type === 'hunter')
        ).toBe(true);
      }
    );
  });
  ...
  it('spawns support groups through the EnemySystem factory pathway', () => {
    ...
    withWaveOverrides(
      ...
      () => {
        ...
        const waveNumber =
          CONSTANTS.SUPPORT_ENEMY_PROGRESSION.hunter.startWave + 1;
...
describe('Combat integration', () => {
  it('routes enemy-fired payloads to the CombatSystem', () => {
    ...
    withWaveOverrides(
      { useManager: false, managerHandlesAsteroids: false },
      () => {
        ...
        enemySystem.handleMineExplosion({
          position: { x: 0, y: 0 },
          radius: 80,
          damage: 50,
        });
...
describe('Reward handling', () => {
  it('drops wave completion bonus XP using RewardManager', () => {
    ...
    const manager = new RewardManager({
      enemySystem,
      xpOrbSystem,
      random: new RandomService('rewards'),
    });
  ...
  it('produces identical support group compositions for identical seeds', () => {
    withWaveOverrides(
      ...
      () => {
        ...
        const managerA = createWaveManager({
          eventBus: eventBusA,
          random: randomA,
        });
        const managerB = createWaveManager({
          eventBus: eventBusB,
          random: randomB,
        });

        const waveNumber =
          CONSTANTS.SUPPORT_ENEMY_PROGRESSION.hunter.startWave + 3;
